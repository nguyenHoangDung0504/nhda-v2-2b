class JaCrawler {
	static crawl() {
		...
		if (Database.getTrackByIdentify(code)) {
			...
			oldTags = Database.getTrackByIdentify(code).tags;
		}
		...
		const cvs = ps[2].textContent
			...
			.map((cv) => {
				...
				if (!rs) return null;
				const testRs =
					cvKeys.get(rs) || Database.getCategory(Database.categoryType.CV, rs.split(' ').reverse().join(' '))?.name;
				return testRs || rs;
			})
		...
		let tags = [...document.querySelectorAll('.post-meta.post-tags a')]
			...
			.map((tag) => {
				let rs = null;
				if ([...tagKeys.values()].includes(tag)) return tag;
		...
		tags = Array.from(new Set([...tags.map((t) => t.split(',')).flat(), ...oldTags]))
			.sort()
			.join(',');
