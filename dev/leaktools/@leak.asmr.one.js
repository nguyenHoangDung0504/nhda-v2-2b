...
async function leakURL(type) {
	...
	const manifest = await (await fetch(`https://api.asmr-200.com/api/tracks/${codePart}?v=1`)).json();
...
function travelFolders(root, path) {
	...
	for (const name of path) {
		...
		currentNode = currentLevel.find((node) => node.type === 'folder' && node.title === name);
