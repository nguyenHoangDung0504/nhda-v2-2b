...
class App {
	...
	static buildHeaderAction() {
		...
		mainSearchIcon.addEventListener('click', () => {
			...
			if (searchValue) {
				...
				if (!developerSearch(searchValue)) {
					window.location = `${window.location.href.includes('s2') ? '/s2' : ''}/?search=${encodeURIComponent(
						searchValue
					)}`;
				}
			}
		...
		function search(value) {
			...
			let suggestions = Utils.filterUniqueObjects(
				splittedValue.reduce((arrOfSuggestion, v) => {
	...
	static buildCategoriesModalView() {
		const [rankListCvCtn, rankListTagCtn, rankListSeriesCtn] = [
			'.rank-list-grid-item.cv-b',
			'.rank-list-grid-item.tag-b',
			'.rank-list-grid-item.series-b',
		].map((selector) => document.querySelector(selector));
		const [listCvCtn, listTagCtn, listSeriesCtn] = [rankListCvCtn, rankListTagCtn, rankListSeriesCtn].map((ctn) =>
			ctn.querySelector('.links')
		);
		...
		[rankListCvCtn, rankListTagCtn, rankListSeriesCtn].forEach(
			(ctn, index) => (ctn.querySelector('.title').textContent += ` (${listLength[index]})`)
		);
		[Database.cvMap, Database.tagMap, Database.seriesMap].forEach((map, index) => {
			map.forEach((value) => {
				const { name, quantity } = value;
				htmls[index] += /*html*/ `<a href="${window.location.href.includes('s2') ? '/s2' : ''}/?${
					types[index]
				}=${encodeURIComponent(name)}" class="item" quantity="${quantity}">${name}</a>`;
			});
		});
				...
				switch (sortTypeSelect.value.toLowerCase()) {
					...
					case 'quantity':
						sortedListOfLinks = Array.from(listOfLinks).sort(
							(a, b) => Number(b.getAttribute('quantity')) - Number(a.getAttribute('quantity'))
						);
						break;
					default:
